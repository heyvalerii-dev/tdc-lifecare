...
import { getAvailableSlotsForDate, SCHEDULING_APPOINTMENT_SELECT, type SchedulingAppointment } from "@/lib/scheduling";
...
export async function GET(request: Request) {
  ...
  if (!psychologistId || !serviceId || !date) {
  ...
  return NextResponse.json(
    slots.map((s) => ({ start: s.start.toISOString(), label: s.label }))
  );
}
